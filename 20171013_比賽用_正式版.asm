...
INITIAL:
  ...
  BCF TRISB,0       ;whether interrtupt at once or not
  BCF LATB,0
  BCF TRISB,1       ;red point with interrupt
  BCF LATB,1
  BSF TRISB,2       ;started by arduino
  BCF TRISB,4       ;direction of wheel
  BCF TRISB,5       ;direction of wheel
  BCF TRISB,6       ;direction of wheel
  BCF TRISB,7       ;direction of wheel
  BCF TRISC,3       ;direction of wheel
  BSF TRISC,4       ;traction B
  BSF TRISC,5       ;traction B
  BCF TRISC,6       ;TX between pic and arduino
  BSF TRISC,7       ;RX between pic and arduino with interrupt
  SETF TRISD        ;traction A & B
  CLRF TRISE        ;direction of wheel
...
START:
  ...
  GOTO LOOP_1
  GOTO START_3
START_3:               ;if it's in the start go forward for seconds
  CALL FORWARD          
  MOVLW duty_70
  MOVWF CCPR1L
  MOVLW duty_64
...
LOOP_1:  
  ...
  CALL INCREASE_22
  NOP
  MOVLW B'00000000'    ;judge where it is
  CPFSEQ BACK_FLAG
...
START_4:              
  DCFSNZ count_revise  ;traction with laser A for few minutes as long as it's going to the next red point
  GOTO START_4_1        
  GOTO JUDGE_1
START_4_1:  
  MOVLW val_revise
...
LOOP_2:                 ;traction with laser B and judge whether laser A is in the red point or not
  CLRF COUNT_INT
  BTFSS PORTD,3
  INCF COUNT_INT 
  BTFSS PORTD,2
...
ACTION_TRACTION:
  SETF BACK_FLAG       ;traction with laser B when laser A is in the red point
  CLRF COUNT_INT
  BTFSS PORTD,3
  INCF COUNT_INT        
  BTFSS PORTD,2
  ...
  CPFSLT COUNT_INT
  GOTO JUDGE_2         
  GOTO LOOP_4          ;when laser A isn't in the red point goto another loop
...
LOOP_4: 
  ...
  CPFSEQ COUNT_INT
  GOTO JUDGE_1 
  GOTO LOOP_RED         ;when laser B is in the red point goto interrupt
  NOP 
;------------------Main program (traction E)--------------
LOOP_7:                  
  CALL FORWARD          ;when it's in long traction always use laser A
  CALL SPEED
  CLRF COUNT_INT
  BTFSS PORTD,3
  INCF COUNT_INT        
...
LOOP_8:
  ...
  GOTO LOOP_8 
  GOTO LOOP_RED 
;-----------------5-Main program (traction F)--------------
LOOP_9:                  ;for blind traction of long traction 
  BTFSS PORTB,2          ;whether it passed the black line or not with arduino
  GOTO LOOP_10
  CLRF TRAC_FLAG
  CALL SELF_CW
  MOVLW duty_60
...
LOOP_RED:
  NOP
  BSF LATB,1              ;goto high priority inrtrrupt
  NOP
  BCF LATB,1
  NOP
  BTFSS INT_FLAG,0
...
JUDGE_RED: 
  BCF INTCON3,INT1IF        ;count the red points
J_1:
  MOVLW B'00000001'
  CPFSEQ COUNT_RED
  GOTO J_2
